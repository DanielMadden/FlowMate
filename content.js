...
(function () {
  ...
  function makePanelChrome({ panel, snapTo, onRemove }) {
    ...
    trashBtn.onclick = () => {
      try { onRemove?.(); } finally { panel.remove(); }
    };
  ...
  function buildASMWidget() {
    ...
    const startLoop = () => {
      ...
      loopInterval = setInterval(async () => {
        const stateText = qs("#sfli-call-header .f9-nowrap-ellipsis span:nth-child(2)")?.textContent?.trim();
        const timeText  = qs("#time-counter .stopwatch-partial")?.textContent?.trim();
        let dialingSeconds = 0;
        if (timeText) {
          const parts = timeText.split(":").map(Number);
          dialingSeconds = parts.length === 3 ? parts[2] : parts[1] || 0;
        }
        const callTypeText = qs("#sfli-call-header .f9-nowrap-ellipsis span:first-child")?.textContent?.trim();
        ...
        if (shouldCountdown) {
          let seconds = (callTypeText === "Agent Call" && stateText === ": Wrap Up") ? 0 : asmDelaySeconds;
    ...
    Object.assign(inputRow.style, { display: "flex", gap: "4px", marginTop: "4px", width: "100%" });

    const delayInput = document.createElement("input");
    Object.assign(delayInput, { type: "number", min: "1", value: asmDelaySeconds, title: "Delay (seconds)" });
    Object.assign(delayInput.style, {
      flex: "1", padding: "6px", border: "1px solid #ccc", borderRadius: "4px",
      fontSize: "12px", background: "#f5f5f5", textAlign: "center", color: "black", minWidth: "0",
    });
    delayInput.onchange = () => { asmDelaySeconds = parseInt(delayInput.value) || 3; };

    const volumeInput = document.createElement("input");
    Object.assign(volumeInput, { type: "number", min: "0", max: "1", step: "0.1", value: asmVolume, title: "Volume (0.0–1.0)" });
    Object.assign(volumeInput.style, {
      flex: "1", padding: "6px", border: "1px solid #ccc", borderRadius: "4px",
      fontSize: "12px", background: "#f5f5f5", textAlign: "center", color: "black", minWidth: "0",
    });
    volumeInput.onchange = () => { asmVolume = parseFloat(volumeInput.value) || 0.5; };

    inputRow.append(delayInput, volumeInput);

    // Assemble
    panel.append(topBar, nextCallBtn, loopBtn, inputRow);
    panel.addEventListener("mouseenter", () => { panel.style.opacity = "1"; stopLoop(); });
  ...
  function buildControlPanel() {
    ...
    const topBar = makePanelChrome({
      ...
      onRemove: () => { stopTabLoop(); stopToastLoop(); },
    });

    const styleBtn = (b, bg) => Object.assign(b.style, {
      margin: "4px 0", width: "100%", padding: "6px",
      border: "none", borderRadius: "4px", background: bg,
      color: "#fff", cursor: "pointer", textAlign: "center", fontSize: "14px",
    });
    ...
    killTabsBtn.onclick = async () => {
      ...
      for (let i = items.length - 1; i >= 0; i--) {
        ...
        if (btn) { btn.click(); await delay(250); }
      }
    };
    ...
    const stopTabLoop = () => { clearInterval(tabLoop); tabLoop = null; syncTab(); };
    ...
    const syncToast = () => {
      toastLoopBtn.textContent = toastLoop ? "Stop Toast Loop" : "Start Toast Loop";
    ...
    const stopToastLoop = () => { clearInterval(toastLoop); toastLoop = null; syncToast(); };
