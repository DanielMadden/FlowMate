...
function FMLog(message) {
  const stack = new Error().stack;
  const callerLine = stack.split("\n")[2].trim(); // 0 = Error, 1 = this function, 2 = caller

  console.log(`[FlowMate] ${message}`);
...
(function () {
  try {
    ...
    const ids = {
      widget: {
        five9: "fm-widget-five9",
        salesforce: "fm-widget-sf",
      },
    };
    ...
    function createPanel({ snapTo, onRemove, content }) {
      const panel = document.createElement("div");
      panel.id = "asm-controls";
      panel.classList.add(["fm-panel"]);
      ...
      // Assemble
      panel.append(createCtrlBar(), ...content);
    ...
    function buildASMWidget() {
      try {
        if (!isFive9) return;
        if (document.getElementById(ids.widget.five9)) return;
        ...
        loopBtn.onclick = () =>
          ASMLoopActive ? stopASMLoop() : startASMLoop();
        ...
        const WidgetFive9 = createPanel({
          snapTo: { bottom: "200px", left: "300px" },
          onRemove: () => stopASMLoop(),
          content: [nextCallBtn, loopBtn, inputRow],
        });

        WidgetFive9.addEventListener("mouseenter", () => {
          stopASMLoop();
        });

        document.body.appendChild(WidgetFive9);
        return WidgetFive9;
      } catch (e) {
        FMLog(e);
      }
    ...
    function buildControlPanel() {
      ...
      killToastsBtn.onclick = () => {
        ...
        const intv = setInterval(() => {
          qsAll(".slds-notify__close .toastClose").forEach((btn) =>
            btn.click()
          );
        }, 500);
        setTimeout(() => {
